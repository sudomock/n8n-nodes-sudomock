...
**Subscriptions** from $0.002 per render. **Without one**, $0.05 per render, the
same rate standalone mockup APIs charge on a paid plan.
A subscription buys a monthly credit allowance plus more template storage on the
higher tiers, and parallel renders are 3 on Starter, 10 on Pro, and 25 on Scale.
The first payment into the balance is at least $5. Photo mockups and video are
priced by what they cost to produce rather than at the flat render rate.
